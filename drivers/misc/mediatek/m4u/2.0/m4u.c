...
static phys_addr_t m4u_user_v2p(unsigned long va)
{
	...
	pud = pud_offset(p4d_offset(pgd, va), va);
...
/* Local replacement for show_pte (static in arm64/mm/fault.c since 5.10) */
static void m4u_show_pte(struct mm_struct *mm, unsigned long addr)
{
	...
	pr_err("m4u_show_pte: addr=0x%lx\n", addr);
	...
	for (i = 0; i < page_num; i++) {
		...
		for (fault_cnt = 0; fault_cnt < 3000; fault_cnt++) {
			if (has_page) {
				...
				if (!pa) {
					handle_mm_fault(vma, va_tmp, (vma->vm_flags & VM_WRITE) ? FAULT_FLAG_WRITE : 0, NULL);
				}
			}
...
static int __maybe_unused MTK_M4U_flush(struct file *filp, fl_owner_t a_id)
{
	return 0;
}
...
static long __maybe_unused MTK_M4U_ioctl(struct file *filp, unsigned int cmd,
			  unsigned long arg)
{
	int ret = 0;
...
static const struct proc_ops m4u_fops = {
	.proc_open = MTK_M4U_open,
	.proc_release = MTK_M4U_release,
	/* .flush, .unlocked_ioctl, .compat_ioctl removed - not in proc_ops */
	/* .mmap = NULL; */
};
