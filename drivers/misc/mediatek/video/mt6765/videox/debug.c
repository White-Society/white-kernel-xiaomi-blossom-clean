...
static ssize_t kick_read(struct file *file, char __user *buf, size_t count, loff_t *ppos);
static ssize_t partial_read(struct file *file, char __user *buf, size_t count, loff_t *ppos);

static const struct proc_ops kickidle_proc_fops = {
    .proc_read = kick_read,
};

static const struct proc_ops partial_proc_fops = {
    .proc_read = partial_read,
};
...
/* proc_ops wrappers for idletime/idlevfp */
static int idletime_proc_show(struct seq_file *m, void *v)
{
    u64 val = 0;
    idletime_get(NULL, &val);
    seq_printf(m, "%llu\n", val);
    return 0;
}
...
{
    return single_open(file, idletime_proc_show, NULL);
}

static ssize_t idletime_proc_write(struct file *file,
...
static const struct proc_ops idletime_proc_fops = {
    ...
    .proc_write = idletime_proc_write,
};

static int idlevfp_proc_show(struct seq_file *m, void *v)
{
    u64 val = 0;
    idlevfp_get(NULL, &val);
    seq_printf(m, "%llu\n", val);
    return 0;
}
...
{
    return single_open(file, idlevfp_proc_show, NULL);
}

static ssize_t idlevfp_proc_write(struct file *file,
    const char __user *buffer, size_t count, loff_t *ppos)
{
    u64 val;
    int ret;
...
void DBG_Init(void)
{
	...
	if (!proc_create("kickdump", S_IFREG | 0440,
		disp_lowpower_proc, &kickidle_proc_fops)) {
	...
	if (!proc_create("partial", S_IFREG | 0440,
		disp_lowpower_proc, &partial_proc_fops)) {
